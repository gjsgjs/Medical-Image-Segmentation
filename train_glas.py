...
# 定义文件夹路径
data_dir = 'Data4AI/Data4CNN/GlaS/allimg'
train_images_dir = os.path.join(data_dir, 'train')
train_annotations_dir = os.path.join(data_dir, 'trainanno')
...
# 创建保存图片的目录
os.makedirs('picss/image', exist_ok=True)
os.makedirs('picss/ground_truth', exist_ok=True)
os.makedirs('picss/predicted', exist_ok=True)
os.makedirs('picss/check_points', exist_ok=True)
        ...
        if (epoch + 1) % 20 == 0:
            ...
            with torch.no_grad():
                ...
                for i in range(8):  # 显示8张图片
                    ...
                    cv2.imwrite(f'picss/image/input_image_img_{i}.jpg', image)
                    cv2.imwrite(f'picss/ground_truth/ground_truth_img_{i}.jpg', annotation)
                
                plt.tight_layout()
                plt.savefig(f'picss/predicted/model_output_epoch_{epoch + 1}.jpg')
                plt.close(fig)

        if (epoch + 1) % 100 == 0:
            torch.save(model.state_dict(), f'picss/check_points/model_epoch_{epoch + 1}.pth')
...
def infer():
    ...
    with torch.no_grad():
        for i in range(len(test_image_paths)):
            ...
            cv2.imwrite(f'picss/predicted/{image_name}', output)
